...
Let $S$ be a $2$-dimensional digital shape. We adopt the cellular-grid model to represent $S$, i.e., pixels and its lower dimensional counterparts, linels and pointels, are part of $S$ (see figure \ref{fig:cellular-grid-model}). In particular, we denote by $\partial S$ the topological boundary of $S$, i.e., the connected sequence of linels such that for each linel we have one of its incident pixels in $S$ and the other not in $S$.
...
Given a closed curve $C_1$ with orientation $q$, and curve $C_2$ with same orientation $q$ such that $C_2$ encloses a region that contains $C_1$, a glued curve is a closed curve  $(c_1,\ell_1,c_2,\ell_2)$ with orientation $q$ and $c_1 \subset C_1, c_2 \subset C_2$. The linels $\ell_1,\ell_2$ are called junction linels.
\end{definition}
...
\begin{align*}
  \hat{\kappa}^2(y) &= c_1\Big( c_2 - |F_{r}^{(i)}(y)| - \sum_{x_j \in O_r^{(i)}(y)} \big( {x_j}  \big) \Big)^2 \\
...
\begin{algorithm}[H]
 ...
 \While{ \It $<$ \MIt  }{ 	
	...
	\If{ \It is odd }{	 
 		...
 		$S^{(i)} \longleftarrow F^{(i-1)} + \overline{X^{(i)}}$\;

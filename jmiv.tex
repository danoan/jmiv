...
Let $d_{S}:\Omega \rightarrow \mathcal{R}$ be the \revision{signed} Euclidean distance transformation with respect to shape $S$. The value $d_S(p)$ gives the Euclidean distance between \revision{$p \notin S$ and the closest pixel in $S$. For points $p \in S$, $d_S(p)$ gives the distance between $p$ and the closest pixel not in $S$.}
...
\center
	...
	\includegraphics[scale=0.125]{images/local_search/flower/h05/summary.eps}
		...
		\caption{\revision{Local combinatorial optimization process results for the square and flower shapes. Shapes displayed at every $2$ iterations for the square and $5$ iterations for the flower.}}	
...
\caption{Elastica computation along iterations of algorithm \ref{alg:local-search}. \revision{The final shape has energy value closer to the optimal value $2\pi/5$ (dashed line)}.}
...
\caption{Running times for the local combinatorial optimization algorithm with $k=1,\revision{n=50}$. Four threads were used.}
...
\begin{algorithm}[H]
 ...
 \While{ \It $<$ \MIt  }{ 	
	...
	\If{ \It is odd }{	 
 		...
 		$S^{(i)} \longleftarrow \revision{F^{(i-1)}_{\overline{S}}} + \overline{X^{(i)}}$\;
...
\includegraphics[scale=0.2]{images/flow/grid-radius-effect/flower/r5-h0.5/summary_flow.eps}}%
		...
		\subfloat[\revision{Elastica evaluation}]{%
...
\begin{figure}[hp!]
	...
	\end{tabular}
	\caption{\revision{Additional comparison results between GrabCut, Schoenemman and Contour correction segmentation methods.}}

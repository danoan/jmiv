...
\revision{We assume an ordering in $\Omega$, i.e., there exists a bijective function $\omega : \Omega \rightarrow \{1 \cdots |\Omega| \}$. Moreover, we associate to every pixel in $\Omega$ a binary variable grouped in the set $X(\Omega)$}

\begin{align*}
	\revision{X(\Omega) := \left\{ x_{\omega(p)} \in \{0,1\} \; | \; p \in \Omega \right\}.}
\end{align*}

\revision{We extend the definition for every subset $P \in \Omega$, i.e.,}

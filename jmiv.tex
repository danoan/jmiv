...
Let $d_{S}:\Omega \rightarrow \mathcal{R}$ be the signed Euclidean distance transformation with respect to shape $S$. The value $d_S(p)$ gives the Euclidean distance between $p \notin S$ and the closest pixel in $S$. For points $p \in S$, $d_S(p)$ gives the distance between $p$ and the closest pixel not in $S$. The \emph{$m$-level set of $S$}
...
\begin{align*}
	\mathcal{R}_m(S) &:= \mathcal{L}_m \cup \mathcal{L}_{-m},
\end{align*}
where
\begin{align*}
	\mathcal{L}_m(S) &:= \left\{ \quad \begin{array}{cc}
...
\center
	...
	\caption{The blue pixels in figure (a) illustrates a $3$-ring set. In figure (b) we highlight the contour of an element of set $\mathcal{N}_{11}$.}
\end{figure}
...
\center
	...
	\includegraphics[scale=0.125]{images/local_search/square/h1/summary.eps}
	}%
	\hspace{10pt}
	\subfloat[h=0.5\label{}]{%
	\includegraphics[scale=0.125]{images/local_search/square/h05/summary.eps}
	}%
	\hspace{10pt}
	\subfloat[h=1.0\label{}]{%
	\includegraphics[scale=0.125]{images/local_search/flower/h1/summary.eps}
	}%
	\hspace{10pt}
	\subfloat[h=0.5\label{}]{%
	\includegraphics[scale=0.125]{images/local_search/flower/h05/summary.eps}
	}%
		\caption{Local combinatorial optimization process results for the square and flower shapes. Shapes displayed at every $2$ iterations for the square and $5$ iterations for the flower.}	
...
experiments in this paper were executed on a $32$-core $2.4Ghz$ CPU and the number of pixels in the square and
flower shapes are respectively $841,1867$ for grid step $h=1.0$. Although its use in practical applications is
limited, we demonstrate that digital estimators are effective in their measurements and the flows evolve as expected. In particular, the digital elastica energy for the experiments in figure \ref{fig:local-comb-square-results} approaches the global optimum (figure  \ref{fig:plot-elastica-local-search}). We
...
\caption{Elastica computation along iterations of algorithm \ref{alg:local-search}. The final shape has energy value closer to the optimal value $2\pi/5$ (dashed line).}
...
\hline
\end{tabular}
\caption{Running times for the local combinatorial optimization algorithm with $k=1,N=50$. Four threads were used.}
...
Given a digital shape $S \subset \Omega \subset \mathbb{Z}^2$, the goal is to evolve a flow $S^{(i)}$ where $S^{(i+1)}$ has lower digital elastica than its precedent.


We observe that the integral invariant estimator \eqref{eq:curvature_approximation} is originaly designed to be evaluated on the digital boundary of some shape, which is the unknown of our problem. Enforcing this restriction implies to incorporate topological information in the form of constraints or penalization terms in the same lines of \cite{schoenemann09linear}, but such approach would result in a third order model, which we prefer to avoid. Instead, we are going to exploit the II estimator definition in order to create a flow that evolves proportionally to the squared curvature of the current shape.
...
We start by defining the optimization variables.
...
We define $X_{r}^{(i)}(p) := X^{(i)} \cap B_r(p)$ and an analogous definition holds for $F_{r}^{(i)}(p)$. Expanding \eqref{eq:curvature-estimator-pixels}, we get 
\begin{align*}
  \hat{\kappa}^2(p) &= c_1\Big( c_2 - |F_{r}^{(i)}(p)| - \sum_{x_j \in X_r^{(i)}(p)} {x_j} \Big)^2 \\
   &= c_1 \cdot \Big( C + 2\left( |F_{r}^{(i)}(p)| - c_2 \right) \hspace{-2mm}\sum_{x_j \in X_{r}^{(i)}(p)}\hspace{-2mm}{x_j} + \hspace{-2mm}\sum_{x_j \in X_{r}^{(i)}(p)}\hspace{-2mm}{x_j^2} + \hspace{-2mm}\sum_{ \substack{x_j,x_k \in X_{r}^{(i)}(p) \\ j<k} }\hspace{-2mm}{2x_jx_k}  \Big),
\end{align*}
...
\begin{align}
  ...
  & + \sum_{p \in R_m(S)}{ \beta \Big( { (1/2+ |F_{r}^{(i)}(p)|-c_2) \cdot \sum_{x_j \in X_{r}^{(i)}(p)}{x_j} } + \sum_{ \substack{x_j,x_k \in X_{r}^{(i)}(p) \\ j<k} }{x_jx_k} \Big) } \nonumber \\
  & + \sum_{x \in X}{\gamma g(S,x)},
  \label{eq:energy-family}
\end{align}
where $g(S,x)$ denotes a similarity term as distance to initial shape $S$ or data fidelity term and $s(x)$ denotes a length penalization term. Each choice of $m$ generates a different flow, which is generally described in the Digital Curve Evolution (DCE) algorithm \ref{alg:evolution-model}.
...
\begin{algorithm}[H]
 ...
 \While{ \It $<$ \MIt  }{ 	
	...
	\If{ \It is odd }{	 
 		...
 		$S^{(i)} \longleftarrow F^{(i-1)}_{\overline{S}} + \overline{X^{(i)}}$\;
...
We remark that the minimization of \eqref{eq:energy-family} for $S^{(i)}$ is not sufficient to derive the next shape $S^{(i+1)}$ because topolical information is not included in the model. Instead, the minimization of \eqref{eq:energy-family} is used as a guide. Recall that the integral invariant estimator approaches curvature by computing the difference between half of the area
...
suffer from issues that typically arise in models that passes through a discretization stage, such as
...
%%\item{\textbf{Classical work on elastica and Wilmore flow in imaging}}
  %%\begin{itemize}
  %% \item{Level lines based disocclusion \cite{masnou98inpainting} }	%% DONE
 %%  \item{Filling-In by Joint Interpolation of Vector Fields and Gray Levels \cite{ballester01filljoint} } %% DONE
 %%  \item{Eulers Elastica and Curvature Based Inpaintings \cite{chan02elasticainpainting} } %% DONE
 %% \item{Discrete willmore flow \cite{bobenko2005discrete}}
 %%  \item{Introducing Willmore flow into level set segmentation of spinal vertebrae \cite{lim2012introducing}}	 %%% DONE
 %% \end{itemize}
  
%%\item{\textbf{Level-sets}}
 %% \begin{itemize}
  %% \item{Image processing via level set curvature flow \cite{malladi1995image} } %% DONE
  %% \item{Shape Modeling with Front Propagation: A Level Set Approach \cite{malladi1995shape} }	%% DONE
%%	\end{itemize}	
...
%%  \begin{itemize}
  %% \item{Fast global optimization of curvature \cite{zehiry10fast}	} %% DONE
